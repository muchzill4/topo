...
func TestNewDependencyOnDockerCLI(t *testing.T) {
	t.Run("returns installation guidance when Docker is unavailable", func(t *testing.T) {
		dependency := health.NewDependencyOnDockerCLI(&runner.Fake{})
		...
		want := health.DependencyCheckResult{Failure: &health.DependencyCheckFailure{
			...
			Fix:      &health.Fix{Description: "Install a supported container engine. See https://github.com/arm/topo#install-a-container-engine"},
		}}
		assert.Equal(t, want, got)
	})
}
...
func TestNewDependencyOnRemoteDocker(t *testing.T) {
	t.Run("probes the target without checking the host Docker binary", func(t *testing.T) {
		target := ssh.NewDestination("user@example.com")
		dependency := health.NewDependencyOnRemoteDocker(&target, func(_ context.Context, gotTarget ssh.Destination) error {

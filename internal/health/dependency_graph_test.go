...
func TestDependencyGraph(t *testing.T) {
	t.Run("Evaluate", func(t *testing.T) {
		t.Run("reports successful dependencies in the group", func(t *testing.T) {
			...
			graph := health.DependencyGraph{Registry: registry}

			got := graph.Evaluate(context.Background(), []health.DependencyID{bartek.ID, virus.ID})

			if assert.Len(t, got, 2) {
				assert.Equal(t, bartek.ID, got[0].ID)
				assert.Equal(t, bartek.Check(context.Background()), got[0].Result)
				assert.Equal(t, virus.ID, got[1].ID)
				assert.Equal(t, virus.Check(context.Background()), got[1].Result)
			}
		})
		...
		t.Run("reports a failed prerequisite and omits its dependent", func(t *testing.T) {
			...
			got := graph.Evaluate(context.Background(), []health.DependencyID{flour.ID, pizza.ID})

			if assert.Len(t, got, 1) {
				assert.Equal(t, flour.ID, got[0].ID)
				wantResult := flour.Check(context.Background())
